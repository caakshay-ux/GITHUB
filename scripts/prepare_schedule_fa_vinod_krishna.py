...
Inputs:
  - Inception FY2024-25 (10-Sep-2024–31-Mar-2025)
  - Fiscal FY2025-26 (01-Apr-2025–31-Mar-2026)
  - Fiscal FY2026-27 YTD (01-Apr-2026–07-Sep-2026)
  - CY2025 Annual synthesized (YE marks proxied from 31-Mar-2025)
...
INCEPTION = ROOT / "input_Inception_FY2024-25_Vinod_Krishna.csv"
FISCAL = ROOT / "input_Fiscal_Statement_Vinod_Krishna.csv"
FISCAL_FY2627 = ROOT / "input_Fiscal_Statement_FY2026-27_Vinod_Krishna.csv"
ANNUAL = ROOT / "input_Annual_Statement_Vinod_Krishna.csv"
OUT = ROOT / "output" / "Foreign_Assets_Schedule_FA_Vinod_Krishna_AY2026-27.xlsx"
...
NOTES = [
    ("Assessee", "Vinodkrishna Poyyale — IBKR U15388294 — Advisor: MATCAP WEALTH ADVISORS PRIVATE LIMITED"),
    (
        "Account opening",
        "First funding via CHF Electronic Fund Transfer on 27-Aug-2024; statement period from 10-Sep-2024. "
        "First equity purchases on 21-Jan-2025.",
    ),
    (
    ...
    (
        "FY 2026-27 (partial)",
        "Sheets 'Capital Gains FY2026-27' and 'Schedule OS - FY2026-27' cover 01-Apr-2026 to 07-Sep-2026 "
        "(statement end). Full-year FY2026-27 requires Activity Statement through 31-Mar-2027.",
    ),
    (
        "Capital gains",
        "Nil realized stock disposals in FY2024-25 and FY2025-26 (buys / holdings only). "
        "FY2026-27 YTD: multiple equity/ETF disposals (see Capital Gains FY2026-27).",
    ),
    ("NFLX split", "10-for-1 split on 14/17-Nov-2025 applied in FIFO"),
    ("Aliases", "AIR↔AIRd, SIE↔SIEd, SHELL↔R6C0d, ZEAL↔ZEALc normalized for FIFO"),
    ("Multi-ccy", "EUR / JPY / HKD / DKK / CHF converted via IBKR YE FX then Rule 115 USD/INR (editable on CG)"),
]
...
def append_fy2026_27(out: Path) -> dict:
    """Add FY2026-27 YTD (to 07-Sep-2026) CG + OS sheets."""
    ...
    orders = merge_orders(
        ...
        extract_orders(sN),
        transfers_to_buy_orders(
            extract_transfers(sI) + extract_transfers(sF) + extract_transfers(sN),
            [open_inc, open_fy, open_n],
        ),
        transfers_to_out_orders(
            extract_transfers(sI) + extract_transfers(sF) + extract_transfers(sN)
        ),
    ...
    # Avoid double-applying pre-window history already in opening lots
    ...
    _write_formula_cg(
        ...
        "Capital gains — FY 2026-27 YTD (01-Apr-2026 to 07-Sep-2026). Partial year — statement ends 07-Sep-2026. FIFO. Yellow cells editable.",
        cg_rows,
    )
    div_usd, div_inr, int_usd, int_inr = _write_os_fy(
        wb,
        "Schedule OS - FY2026-27",
        "Interest & Dividend income — FY 2026-27 YTD (01-Apr-2026 to 07-Sep-2026). Partial — full year needs statement through 31-Mar-2027.",
        divs,
        ints,
    )
    ...
    if "Notes for CA" in wb.sheetnames:
        wn = wb["Notes for CA"]
        wn.append([])
        wn.append(["FY 2026-27 YTD (01-Apr-2026 to 07-Sep-2026) — partial statement"])
        ...
        wn.append(
            [
                "FY2026-27 coverage",
                "Activity Statement ends 07-Sep-2026 — not full FY. Obtain statement to 31-Mar-2027 for complete year.",
            ]
        )
    ...
    if "CG Summary by Symbol" in wb.sheetnames:
        ...
        wss.append([])
        wss.append(["— FY 2026-27 YTD (to 07-Sep-2026) —"])
...
def run():
    ...
    main(
        ...
        fiscal=FISCAL,
        out=OUT,
        account_open_date="2024-08-27 (first CHF EFT funding)",
        assessee_label="Vinodkrishna Poyyale",
        notes_extra=NOTES,
    )
