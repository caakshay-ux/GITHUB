#!/usr/bin/env python3
"""
Schedule FA / CG / Dividends for Yagyank Chadha — IBKR U15172057 (Individual).

Inputs:
  - FY2024-25 Activity Statement (03-Sep-2024–31-Mar-2025) as inception
  - FY2025-26 Activity Statement (01-Apr-2025–31-Mar-2026) as fiscal
CY2025 Annual synthesized; YE marks proxied from 31-Mar-2025 closes where still held.
Taxable CG: BLBD, NOVd, ORCL, 1810 sales in FY2025-26.
Internal Out of SHOP + Lux UCITS to U20291582 on 23-May-2025 (code I — not taxable).
...
INCEPTION = ROOT / "input_Inception_FY2024-25_Yagyank_U15172057.csv"
FISCAL = ROOT / "input_Fiscal_Statement_Yagyank_U15172057.csv"
FISCAL_FY2627 = ROOT / "input_Fiscal_Statement_FY2026-27_Yagyank_U15172057.csv"
ANNUAL = ROOT / "input_Annual_Statement_Yagyank_U15172057.csv"
OUT = ROOT / "output" / "Foreign_Assets_Schedule_FA_Yagyank_Chadha_U15172057_AY2026-27.xlsx"
...
def synthesize_annual(inception: Path, fiscal: Path, annual_out: Path) -> None:
    ...
    all_xfers = extract_transfers(sI) + extract_transfers(sF)
    orders = merge_orders(
        extract_orders(sI),
        extract_orders(sF),
        transfers_to_buy_orders(all_xfers, [open_inc, open_fy]),
    ...
    start_nav = round(
        sum(
            to_usd(sum(L.cost_local for L in lots), (lots[0].currency if lots else "USD"))
            for lots in lots_open_cy.values()
            if lots
        ),
        2,
    )
    ...
    add(
        ...
        "Data",
        "WhenGenerated",
        "Synthesized for Schedule FA from FY Activity Statements; YE marks proxied from 31-Mar-2025 (see Notes)",
    )
...
NOTES = [
    (
        "Assessee",
        "Yagyank Chadha — IBKR U15172057 (Individual) — "
        "Advisor: MATCAP WEALTH ADVISORS PRIVATE LIMITED "
        "(also spelled Chaddha in some correspondence)",
    ),
    (
        "Account opening",
        "First funding Sep-2024 (EFTs). FOP In Sep-2024: SHOP × 84, "
        "Fidelity LU0099574567 × 511.42, BGF LU0106831901 × 792.45. Multi-currency equities/ETFs.",
    ),
    (
        "Related account",
        "Internal Out 23-May-2025 to U20291582: SHOP × 84 + both Lux UCITS — "
        "excluded from taxable CG (code I). See U20291582 workbook.",
    ),
    (
        "Capital gains FY2025-26",
        "Taxable sales: BLBD × 24 (11-Aug-2025), NOVd × 90 (24-Nov-2025), "
        "ORCL × 5 (25-Nov-2025), 1810 × 600 (21-Dec-2025). Internal Outs not taxable.",
    ),
    (
        "FY 2026-27 (partial)",
        "Sheets 'Capital Gains FY2026-27' and 'Schedule OS - FY2026-27' cover 01-Apr-2026 to 08-Sep-2026 "
        "(statement end). Full-year FY2026-27 requires Activity Statement through 31-Mar-2027. "
        "Includes APH 2-for-1 split (03-Sep-2026).",
    ),
    (
        "Annual CY2025 / YE marks",
        "No Calendar-Year Annual furnished. CY2025 synthesized from FY statements. "
        "YE2025 closes proxied from 31-Mar-2025 marks where still held, else cost — "
        "prefer IBKR Annual / PortfolioAnalyst for filing.",
    ),
    (
        "FX",
        "Non-USD holdings (EUR/HKD/JPY/DKK/TWD) converted via IBKR YE/cross rates then Rule 115. "
        "Editable on FX Lookup / CG sheets.",
    ),
]
...
def append_fy2026_27(out: Path) -> dict:
    """Add FY2026-27 YTD (to 08-Sep-2026) CG + OS sheets."""
    ...
    _write_formula_cg(
        ...
        "Capital gains — FY 2026-27 YTD (01-Apr-2026 to 08-Sep-2026). Partial year — statement ends 08-Sep-2026. FIFO. Yellow cells editable.",
        cg_rows,
    )
    div_usd, div_inr, int_usd, int_inr = _write_os_fy(
        wb,
        "Schedule OS - FY2026-27",
        "Interest & Dividend income — FY 2026-27 YTD (01-Apr-2026 to 08-Sep-2026). Partial — full year needs statement through 31-Mar-2027.",
        divs,
        ints,
    )
    ...
    if "Notes for CA" in wb.sheetnames:
        wn = wb["Notes for CA"]
        wn.append([])
        wn.append(["FY 2026-27 YTD (01-Apr-2026 to 08-Sep-2026) — partial statement"])
        ...
        wn.append(
            [
                "FY2026-27 coverage",
                "Activity Statement ends 08-Sep-2026 — not full FY. Obtain statement to 31-Mar-2027 for complete year.",
            ]
        )
    ...
    if "CG Summary by Symbol" in wb.sheetnames:
        ...
        wss.append([])
        wss.append(["— FY 2026-27 YTD (to 08-Sep-2026) —"])
...
def run():
    ...
    main(
        ...
        fiscal=FISCAL,
        out=OUT,
        account_open_date="2024-09-03 (first USD EFT funding)",
        assessee_label="Yagyank Chadha (U15172057)",
        notes_extra=NOTES,
    )
