...
def synthesize_annual(src: Path, annual_out: Path) -> None:
    ...
    # YE2025 mark proxy: inbound ACATS/FOP MV÷qty (near YE for AVGO ACATS 18-Dec-2025); else cost.
    close_px: dict = {}
    for tfer in extract_transfers(s):
        if tfer["direction"].lower() != "in":
            continue
        sym = normalize_symbol(tfer["symbol"])
        qty = tfer["qty"]
        mv = tfer.get("market_value")
        if qty and mv:
            close_px[sym] = (abs(mv) / qty, tfer.get("currency") or "USD")
    ...
    add(
        ...
        "Data",
        "WhenGenerated",
        "Synthesized for Schedule FA from FY statement (opened Nov-2025); YE AVGO mark from ACATS (see Notes)",
    )
...
NOTES = [
    ("Assessee", "Rajani J Vallath — IBKR U16003525 (Individual) — Advisor: MATCAP WEALTH ADVISORS PRIVATE LIMITED"),
    (
        "Account opening",
        "Funded 18-Nov-2025 with USD 1 EFT. AVGO × 250 received via ACATS In on 18-Dec-2025 "
        "(from external account 27960235; carryover cost ~USD 62.772/share; ACATS MV USD 81,505).",
    ),
    (
    ...
    (
        ...
        "No Calendar-Year Annual furnished. CY2025 synthesized. YE2025 AVGO closing proxied from ACATS transfer price "
...
def run():
    ...
    main(
        ...
        fiscal=FISCAL,
        out=OUT,
        account_open_date="2025-11-18 (first USD EFT USD 1)",
